...
import '../presenter/scanner_presenter.dart';
import '../view/error_popup.dart';
...
class ScannerState extends State<Scanner> {
  ...
  Widget getTallyApprovalDialog(scanData) {
    ...
    return AlertDialog(
      title: const Text('Start a New Tally'),
      content: SingleChildScrollView(
        child: ListBody(
          children: <Widget>[
            Text('Would you like to start a tally with ' +
                (tally.friend?.displayName ?? "-") +
                "?"),
          ],
        ),
      ),
        ...
        TextButton(
            ...
            onPressed: () {
              ...
              if (presenter.registerNewTally(ticket)) {
                ...
                userTallies!.add(tally);
                AppContext().user!.transactionList[tally.personID] = [];
                ...
                errPop(context,
                    "Couldn't create a tally because [INSERT REASON HERE]");
            }),
      ],
    );
  }
  ...
  void transitionToNewTally(tally) {
    Navigator.push(context,
        MaterialPageRoute(builder: (BuildContext context) => TallyPage(tally)));
  }
