...
describe("Peer to peer tallies", function() {
  ...
  it("Check for correct number of test users", function(done) {
    const sql = `begin; \
      ...
      update mychips.users_v set peer_inet = '${MachineIP}', peer_port=${Pport0}, host_id='${Host0}' where peer_ent = 'p1000'; \
      update mychips.users_v set peer_inet = '${MachineIP}', peer_port=${Pport1}, host_id='${Host1}' where peer_ent = 'p1001'; \
  ...
  it("User p1001 proposes a tally to user p1000", function(done) {
    const sql = `begin;
      ...
      insert into mychips.tallies (tally_ent, tally_guid, partner, user_sig, contract, request) values ('p1001', '18d44de5-837d-448f-8d96-9136372987cf','p1000','Adam signature', '{\"name\":\"mychips-0.99\"}'::jsonb, 'draft') returning status;
      commit;`
...
//console.log("Contract:", msg.contract, typeof msg.contract)
      assert.equal(msg.contract.name, 'mychips-1.0')
...
//      assert.equal(msg.signed.foil, null)
  ...
  it("User p1001 requests payment from user p1000", function(done) {
    const sql = `begin;
      ...
      insert into mychips.chits (chit_ent, chit_seq, chit_guid, chit_type, units, pro_quo, request) values ('p1001', 1, 'd0921c68-de42-4087-9af1-0664605d4136', 'tran', 12345600, 'Consulting', 'userRequest') returning units;
      commit;`
    db1.query(sql, null, (err, res) => { if (err) done(err)
  ...
  it("User p1001 sends partial refund to user p1000", function(done) {
    const sql = "begin; \n\
      insert into mychips.chits (chit_ent, chit_seq, chit_guid, chit_type, units, pro_quo) values ('p1001', 1, '1a7e3036-7f3e-40f7-b386-0af972ee77f5', 'tran', -2345600, 'Partial refund'); \n\
      update mychips.chits set request = 'userDraft', signature='Adam Signature' where chit_ent = 'p1001' and signature is null returning units; commit;"
    db1.query(sql, null, (err, res) => { if (err) done(err)
      var units = res[2].rows[0]['units']
