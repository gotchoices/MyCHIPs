...
describe("Peer-to-peer lift testing", function() {
  ...
  before('User connection to test database', function(done) {
    dbU = new dbClient(new dbConf(log,user0Listen), (chan, data) => {
...
//log.debug("Sql:", sql, parms)
    ...
    busL.register('pa', (msg) => {		//Listen for admin notification
      assert.equal(msg.target, 'tallies')	//;log.debug("A msg:", msg)
      assert.equal(msg.oper, 'UPDATE')
      _done()
    })
    busU.register('pu', (msg) => {		//Listen for user notification
      assert.equal(msg.target, 'lift')		//;log.debug("A msg:", msg)
      assert.equal(msg.state, 'good')
      assert.equal(msg.entity, user0)
