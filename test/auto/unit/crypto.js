const { testLog, libModule, timeShort } = require('../common')
...
const McCrypto = require(libModule('crypto.js'))
...
describe('TallyCrypto Class', function() {
  let logErrorStub
  this.timeout(timeShort)
  ...
  describe('Generate method', function() {
    it('Should generate key pair and call the callback with keys', (done) => {
      const tallyCrypto = new McCrypto(log)

      tallyCrypto.generate().then(({keys, priv, publ}) => {
    ...
    it('should handle errors gracefully and call the error log', (done) => {
      ...
      tallyCrypto.generate().then(() => {
        assert.fail('Expected generate to throw an error')
        done()
      }).catch((err) => {
        try {
          assert.strictEqual(err, error)
          assert(logErrorStub.calledWith('Crypto generate: Test error'))
          done()
        } catch (aErr) {
          done(aErr)
        } finally {
  ...
  describe('Sign method', () => {
    it('should sign a message and call the callback with the signature for string key', (done) => {
      ...
      tallyCrypto.sign(fakeKeyString, fakeMessage).then(signature => {
    ...
    it('should sign a message and call the callback with the signature for JSON key', (done) => {
      ...
      tallyCrypto.sign(fakeKeyObject.privateKey, fakeMessage).then(signature => {
        assert.deepStrictEqual(signature, fakeSignature)
        importKeyStub.restore()
        signStub.restore()
        done()
    ...
    it('should handle errors gracefully and call the error log', (done) => {
      ...
      tallyCrypto.sign(fakeKeyObject.privateKey, fakeMessage).then(() => {
        assert.fail('Expected sign to throw an error')
        done()
      }).catch((err) => {
        try {
          assert.strictEqual(err, error)
          assert(logErrorStub.calledWith('Crypto sign: Test error'))
          done()
        } catch (aErr) {
          done(aErr)
        } finally {
  ...
  describe('verify', () => {
    it('should verify a valid signature and call the callback with true', (done) => {
      ...
      tallyCrypto.verify(fakeKeyObject.publicKey, fakeMessage, fakeSignature).then(isValid => {
    ...
    it('should verify an invalid signature and call the callback with false', (done) => {
      ...
      tallyCrypto.verify(fakeKeyObject.privateKey, fakeMessage, fakeSignature).then(isValid => {
    ...
    it('sould handle string and object argments', (done) => {
      ...
      tallyCrypto.verify(fakeKeyString, fakeMessage, fakeSignature).then(isValid => {
        assert.strictEqual(isValid, true)
        importKeyStub.restore()
        verifyStub.restore()
        done()
    ...
    it('should handle errors gracefully and call the error log', (done) => {
      ...
      tallyCrypto.verify(fakeKeyObject.publicKey, fakeMessage, fakeSignature).then(() => {
        assert.fail('Expected verify to throw an error')
        done()
      }).catch((err) => {
        try {
          assert.strictEqual(err, error)
          assert(logErrorStub.calledWith('Crypto verify: Test error'))
          done()
        } catch (aErr) {
          done(aErr)
        } finally {
