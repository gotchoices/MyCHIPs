...
const userSql = `select id, std_name, ent_name, fir_name, ent_type, user_ent,
    peer_cid, peer_agent, peer_host, peer_port,
...
module.exports = class {
  constructor(sqlConfig, docConfig, opts) {
    ...
    if (!(this.agent = opts.agent)) {
      this.log.error("Must supply an agent name")
      process.exit()
    }
    this.log.info('Initializing agent-based model controller 2 for:', this.agent)
    console.log('STARTING MODEL * VERSION 2 *')
    ...
    if (opts.runs) {
      this.runs = opts.runs   //Max iterations
...
//      this.docAcStream = this.docAc.watch([{$match: { agent: null }}])
      this.docAcStream = this.docAc.watch([{$match: { 'fullDocument.agent': this.agent }}])	//Receive async updates for this agent
      this.docAcStream.on('error', ev=>{this.log.error("Couldn't watch mongo:", this.agent, ev)})

      this.docAcStream.on('change', ev=>{		//Handle async notices from doc DB
        let doc = ev ? ev.fullDocument : {}
this.log.debug("Got doc change:", doc.action, doc.agent, doc.data)

//        if (doc.action == 'createUser') {		//Someone asking me to insert a peer into the DB
//          this.checkPeer(doc.data, pDat=>{
//this.log.debug("Peer added/OK:", pDat.peer_cid, "notifying:", doc.data.agent)
//            this.docAc.insertOne({action: 'done', tag:doc.tag, host:doc.data.agent, from:this.agent}, ()=>{})
//          })
//        } else 
        if (doc.action == 'done') {		//Remote action has completed
...
// -----------------------------------------------------------------------------
  remoteCall(agent, cmd, data, cb) {			//Execute command on foreign peer
    let tag = agent + '.' + this.remoteIdx++		//Make unique message ID
this.log.debug("Remote calling:", cmd, "to:", agent)
    this.remoteCBs[tag] = cb				//Remember function to call locally on completion
    this.docAc.insertOne({action: cmd, tag, agent, from: this.agent, data}, null, (err, res)=>{
      if (err) this.log.error("Sending remote command:", cmd, "to:", agent)
    })
  }
  ...
  eatParms(err, res) {			//Digest updated operating parameters from database
    ...
    this.intervalTimer = setInterval(()=>{
      if (this.entity != null && (!this.runs || this.counter < this.runs)) {
        console.log("### RUN NUMBER", ++this.counter, "###")
        this.process(this.counter)
//Stay alive for simulator
//      } else {
//        this.close()
...
//        , vData = vId ? this.entities.find(el=>(el.id == vId)) : null
      this.log.debug("  L:", aData.peer_cid, "pays vendor", idx, '/', aData.vendors.length, aData.vendor_cids[idx], "NW:", aData.units)
...
this.log.verbose('  Tally offer for:', msg.entity, msg.sequence)
      let target = 1				//Immediately eligible for small lift
        , sql = `update mychips.tallies_v set request = 'open', hold_sig = $3, target = $4
          where tally_ent = $1 and tally_seq = $2 returning state;`
...
this.log.verbose('  Tally accepted:', row.tally_ent, row.tally_seq, row.state)
      })
    }
  }
  ...
  payVendor(aData, idx) {
    let uuid = Uuid.v4()
      ...
      , sql = `insert into mychips.chits_v (
          chit_ent, chit_seq, chit_uuid, signature, units, quidpro, request
        ) values ($1, $2, $3, $4, $5, $6, $7)`
