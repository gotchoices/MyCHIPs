...
const MakeNewTally = (agentData, worldDBFacade, hostDBFacade, logger) => {
  ...
  this.shouldProcess = () => {
    return agentData.stocks <= 0 ||                 // If the agent has no stocks   or
      (agentData.stocks < this.parms.maxstocks &&   // (if the agent doesn't have too many stocks and
        Math.random() < this.parms.addclient)       //  we randomly choose to)
  }
  ...
  var tryTally = (agentData, peerWorldData) => {
