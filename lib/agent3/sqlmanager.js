...
module.exports = class SQLManager {
  ...
  constructor(sqlConfig, logger, host) {
    this.#logger = logger
    this.#host = host
  ...
  queryUsers(callback, all) {
    this.query(userSql, (err, res) => {
      if (err) {
        this.#logger.error('In query:', err.stack)
        return
      }
      callback(res.rows, all)
    })
  }

  queryLatestUsers(time, callback, all) {
  ...
  queryPeers(peerData, port, callback) {
    this.query(
      peerSql,
      [
        peerData.ent_name,
