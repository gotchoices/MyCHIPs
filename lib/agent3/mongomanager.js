...
class MongoManager {
    ...
    createConnection(options, checkPeer, notifyOfActionDone, loadInitialUsers) {
        ...
        this.dbConnection = new mongodb_1.MongoClient(url, options);
    ...
    insertAction(cmd, tag, host, data) {
        this.actionsCollection.insertOne({ action: cmd, tag, host, from: this.host, data }, null, (err, res) => {
            if (err)
                this.logger.error('Sending remote command:', cmd, 'to:', host);
        });
    }
    updateOneAgent(row) {
    ...
    deleteManyAgents(haveEm) {
        this.agentsCollection.deleteMany({
            ...
            if (e)
                ...
                this.logger.debug('Delete agents in world:', r.n);
        });
    }
    updateAgents() { }
}
