...
class MongoManager {
    ...
    insertAction(command, tag = this.host + '.' + this.foreignActionTagIndex++, destinationHost, data, callback) {
        ...
        this.actionsCollection.insertOne({ action: command, tag: tag, host: destinationHost, from: this.host, data: data }, (err, res) => {
            if (err) {
    ...
    findPeerAndUpdate(hostedAccountPeerCid, alreadyConnectedAccounts, callback) {
        ...
        // @ts-ignore
        this.accountsCollection.findOneAndUpdate(query, update, options).then((res) => {
            var _a, _b;
            if (res.ok) {
