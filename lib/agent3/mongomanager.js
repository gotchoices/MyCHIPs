...
class MongoManager {
    ...
    createConnection(options, notifyOfNewAgentRequest, loadInitialUsers) {
        let url = `mongodb://${this.dbConfig.host}:${this.dbConfig.port}/?replicaSet=rs0`;
        this.logger.verbose('Mongo:', this.host, url);
        this.mongoClient = new mongodb_1.MongoClient(url, options);
    ...
    insertAction(cmd, tag = this.host + '.' + this.foreignActionTagIndex++, destinationHost, callback) {
        this.actionsCollection.insertOne({ action: cmd, tag: tag, host: destinationHost, from: this.host }, (err, res) => {
            if (err)
                this.logger.error('Sending remote command:', cmd, 'to:', destinationHost);
            else
                this.logger.info('Sent remote action:', cmd, 'to:', destinationHost);
        });
        if (callback) {
