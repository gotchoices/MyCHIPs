...
$$;
create function mychips.peers_vf_agent(new mychips.peers_v, old mychips.peers_v, tgop text) returns mychips.peers_v language plpgsql security definer as $$
    begin
...
create function mychips.peers_vf_post(new mychips.peers_v, old mychips.peers_v, tgop text) returns mychips.peers_v language plpgsql security definer as $$
    begin
...
create function mychips.peers_v_insfunc() returns trigger language plpgsql security definer as $$
  declare
    ...
    str  varchar;
  begin
    nrec = new; new = mychips.peers_vf_agent(nrec, null, TG_OP); if new is null then return null; end if;
...
create function mychips.peers_v_updfunc() returns trigger language plpgsql security definer as $$
  ...
  begin
    ...
    select into new * from mychips.peers_v where id = new.id;
    nrec = new; orec = old; new = mychips.peers_vf_post(nrec, orec, TG_OP);
    return new;
  end;
$$;
...
insert into base.parm (module, parm, type, v_int, v_text, cmt) values 
 ...
 ,('mychips', 'peer_agent', 'text', null, 'DEFAULT_AGENT', 'Default agent key where peer servers connect.')
