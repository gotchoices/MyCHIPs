...
$$;
create function mychips.agents_tf_bi() returns trigger language plpgsql security definer as $$
    begin
      new.agent_key = decode(replace(replace(new.agent,'-','+'), '_','/'), 'base64');
      return new;
    end;
$$;
create function mychips.agents_tf_bu() returns trigger language plpgsql security definer as $$
    begin
...
$$;
create trigger mychips_agents_tr_bi before insert on mychips.agents for each row execute procedure mychips.agents_tf_bi();
create trigger mychips_agents_tr_bu before update on mychips.agents for each row execute procedure mychips.agents_tf_bu();
...
peer_ent	text		primary key references base.ent on update cascade on delete cascade
  ...
  , peer_hid	text		
  , peer_agent	text	        references mychips.agents on update restrict
...
create view mychips.peers_v as select 
  ...
  , pe.peer_ent, pe.peer_cid, pe.peer_hid, pe.peer_psig, pe.peer_cmt, pe.crt_by, pe.crt_date, pe.mod_by
  ...
  , pe.peer_cid || '@' || coalesce(ag.agent_host, pp.phost)				as peer_addr
  , pe.peer_cid || '@' || coalesce(ag.agent_host, pp.phost) || ':' || coalesce(ag.agent_port, pp.pport)	as peer_endp
...
create view mychips.users_v as select 
  ...
  , pe.peer_ent, pe.peer_cid, pe.peer_hid, pe.peer_psig, pe.peer_cmt
...
create function mychips.peers_v_insfunc() returns trigger language plpgsql security definer as $$
  ...
  begin
    ...
    execute 'select ' || str || ';' into trec using new;
    insert into mychips.peers (peer_ent,peer_cid,peer_hid,peer_psig,peer_cmt) values (new.id,trec.peer_cid,trec.peer_hid,trec.peer_psig,trec.peer_cmt);
...
insert into base.parm (module, parm, type, v_int, v_text, cmt) values 
 ...
 ,('mychips', 'peer_host', 'text', null, 'mychips.org', 'Default IP where peer servers connect.')
