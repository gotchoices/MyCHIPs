...
this.log.debug('New PC; peer:', chad, this.state)
...
this.log.debug('Got I packet:', data, data.ticket)
...
this.log.debug('Ate packet:', header, data)
  ...
  peerAddress() {
this.log.debug("Peer address:", this.agent)
    if (!this.fromAddress) {
...
this.log.debug("Ending peer connection:", this.agent)
...
this.log.debug('Initializing noise:', this.manager.keys.publicKey, this.peerKey, messageObj)
    if (ticket) {
      messageObj.from = this.manager.chad		//Assure valid from address on packet
...
this.log.debug('Peercomm send to:', agent, host, port)
    ...
    let pc = this.connections.find(agent, (v,o)=>(v==o.agent))		//Find any existing connection
