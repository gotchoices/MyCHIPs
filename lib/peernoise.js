...
//X- Are encrypt and manager.key switches redundant?	Yes, get rid of encrypt switch
//X- Implement non-encrypted mode (for simulations)
...
//- Delete manager peer key cache entry if the key ever fails
...
//    this.queue = []				//pending messages; Fixme: do we need this?
    ...
    this.raw = !('secretKey' in manager.key)
    this.n = new NoiseWrap({log: this.log, raw: this.raw})
    
    if (peer && !socket) {			//The manager must initiate the connection using this.send()
      this.setPeer(peer)
...
this.log.debug('preNoise:', hs1, handshake, this.peerKey, peerKey)
      this.noiseState = this.n.initialize(handshake, initiator, this.manager.key, peerKey)
    }
...
this.log.debug('Got I packet:', data)
          ...
          this.manager.query('ticket', ticket, () => {	//Have app validate ticket
            this.setPeer(ticket.peer)		//Record the peer address:port
            this.spewNoise()			//Acknowledge completes noise connection
            this.state = 'open'
this.log.debug('PK:', JSON.stringify(ticket.peerKey), JSON.stringify(this.noiseState.rs))
            this.peerKey = this.n.keyCheck(this.noiseState, ticket.peerKey)	//Make copy of peer's public key
...
this.log.debug('Ticket initiate:', ticket)
      this.peerKey = ticket.key		//Remember the remote peer's public key
...
this.log.debug('No peer key, try to query app')
      return this.manager.query('key', this.peer, key => {
...
this.log.debug('Initializing noise:', this.manager.key, this.peerKey, messageObj)
    let hs1
    if (ticket) {
      ticket.peer = this.manager.peer		//Add ID, key piggyback on message
      ticket.peerKey = this.manager.publicKey
      ...
      this.spewNoise(header, JSON.stringify(this.manager.peer), ns)	//Our ID
      this.n.destroy(ns)
      hs1 = 'K'					//First character of handshake
    }
    let messageBuf = Buffer.from(JSON.stringify(messageObj))
      , packetBuf = Buffer.alloc(messageBuf.byteLength + this.n.BufferAdd)
      , noiseBuf = packetBuf.slice(1)		//Noise starts after header character

    this.noiseState = this.n.initialize(hs1 + 'K', this.initiator = true, this.manager.key, this.peerKey)
...
this.log.trace('Writing bytes:', this.n.writeBytes, packetBuf.toString('hex'))
...
this.log.debug('Decrypted:', header, decryptBuf.toString())
  ...
  encrypt(messageObj, successCB, failureCB) {
    ...
    if (!this.raw && (!this.split || !this.split.tx)) {
      ...
      sendBuf.write('T')
this.log.debug('Write:', sendBuf.toString(this.raw ? 'utf-8' : 'hex'))
...
module.exports = class {
  constructor(config, messageCB) {
    ...
    this.key = config.key
    this.publicKey = config.key.publicKey
...
this.log.debug('Peer initiated connection on:', this.port, ' from:', sock.remoteAddress)
  ...
  //---- Get/set cached value for peer public key
  peerKey(peer, value) {
...
//this.log.debug('  length now:', this.connections.size())
  ...
  send(peer, messageObj, successCB, failureCB) {
