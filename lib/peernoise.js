...
this.log.debug('New PC; addr:', socket.remoteAddress, 'Raw:', this.raw)
  ...
  setPeerInfo({agent, host, port, key}) {
    ...
    this.peerHost = host
    this.peerPort = port
    this.peerKey = key ?? this.manager.query('noiseKey', agent)
this.log.trace('PeerInfo:', agent, host, port, this.peerKey)
  }
      ...
      if (this.state == 'open') {			//Established connection
                ...
                try {
                  ...
                  if (data && Object.keys(data).length >= 1) {	//Process message if any
                    try {
                      ...
                      this.log.error('Error in message callback:', err.message)
                    }
                  }
                } catch (err) {
                  this.log.error('Error processing I ticket:', err.message)
                }
              })	//query
            }	//if header
          } catch (err) {
            this.log.error('Error processing noise packet:', err.message)
          }
        })	//eatNoise()
        ...
        this.eatNoise(packet, false, ['K'], (header, data) => {
          try {
            ...
            this.log.error('Error processing K packet:', err.message)
          }
        })
        
      } else {		//state = request or ticket
        this.log.error('Unexpected packet state:', this.state)
      }
    } catch (err) {
      this.log.error('Unhandled error in fromInitiator:', err.message)
    }
  }		//fromInitiator
    ...
    try {
      ...
      if (target == 'tally' && action == 'ticket' && ticket) {
        ...
        try {
          ...
          this.log.error('Error sending node ID:', err.message)
      ...
      try {
        ...
        this.log.error('Error in noise protocol initialization:', err.message)
        if (failureCB) failureCB(err)
      }
    } catch (err) {
      this.log.error('Unhandled error in initiate method:', err.message)
      if (failureCB) failureCB(err)
    }
  }
        ...
        try {
          ...
          if (messageObj) {
            try {
              ...
              this.log.error('Error in message callback after decrypt:', err.message)
            }
          } else {
            this.log.error('Failed to parse decrypted message')
          }
        } catch (err) {
          this.log.error('Error during decryption:', err.message)
        }
      }
    } catch (err) {
      this.log.error('Unhandled error in decrypt method:', err.message)
    }
  }
...
this.log.trace('In PC send; state:', this.state, messageObj)
...
module.exports = class {
  constructor(config, messageCB) {
    ...
    this.queryCB = config.queryCB			//Call to query for keys, tokens, tallies
...
this.log.debug('Peer initiated connection on:', this.port, ' from:', sock.remoteAddress)
  ...
  query(command, data, cb) {
    if (!this.queryCB) return null
    return this.queryCB(command, data, cb)
  }
  ...
  send(messageObj, successCB, failureCB) {
    try {
      let {to} = messageObj
        , {agent, host, port} = to
      this.log.debug('Peernoise send to:', agent, host, port)
      if (!agent || !host || !port) {
        if (failureCB) return failureCB(Error('Bad agent, host or port'))
        return
      }
      ...
      if (agent == this.agent) {				//This is local traffic
        setTimeout(() => {
          try {
            ...
            this.log.error('Error in local message callback:', err.message)
            if (failureCB) failureCB(err)
          }
        })
        return
      ...
      if (!pc) {						//Build a connection if necessary
        ...
        try {
          ...
          this.log.error('Error creating peer connection:', err.message)
          if (failureCB) failureCB(err)
        }

      } else {						//Already have a peer connection object
        this.log.trace('Found existing control object for peer:', agent)
        pc.send(messageObj, successCB, failureCB)
      }
    } catch (err) {
      this.log.error('Unexpected error in send method:', err.message)
      if (failureCB) failureCB(err)
    }
  }
