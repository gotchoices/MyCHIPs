...
const AgentFactory = require('./agent3/agentFactory')
...
module.exports = class AgentCluster {
  ...
  constructor(myChipsDBConfig, worldDBConfig, argv) {
    ...
    // Load up data from Databases
    this.load()
  }

  configureDatabases(myChipsDBConfig, worldDBConfig) {
    // Configure MyCHIPs DB
    this.#myChipsDBManager = new SQLManager(myChipsDBConfig, this.#logger, this.#host)
    // Configure World DB
    this.#worldDBManager = new MongoManager(worldDBConfig, this.#logger, this.#host)

    // Connect to MyCHIPs DB
    this.#myChipsDBManager.createConnection(this.#params, this.notifyOfAgentsChange,
      this.notifyOfParamsChange, this.notifyOfTallyChange)
    // Connect to World DB
    this.#worldDBManager.createConnection(WorldDBOpts, this.checkPeer, this.notifyOfActionDone)
  }

  // Runs after both databases have been configured. Loads up initial data from the DBs
  load() {
    // Load Users from the MyCHIPs DB and put them into the World DB
  ...
  run() {
    this.#currAgent = null //Keep track of which user we are processing //TODO remove
    this.#counter = 0
    if (this.#argv.runs) {  // Max iterations. If runs isn't provided, we will run forever.
      this.#runs = this.#argv.runs
    } else if (this.#params.runs) {
      this.#runs = this.#params.runs
    }
    ...
    //   })
    // }
    // Set up interval to call process() every `interval` milliseconds
    this.#intervalTimer = setInterval(() => {
      if (!this.#runs || this.#counter < this.#runs) {
  ...
  loadInitialUsers() {
    console.log("loading intitial users");
    this.#myChipsDBManager.queryUsers(this.eatAgents, true) 
  }
  ...
  // Builds Agent objects for the local agents
  buildAgents() {
    this.#params.agentTypes.forEach(config => {
      // TODO: make this % calculation more robust
      let numOfType = config.percentOfTotal * this.#myAgentsInfo.size
      for (let i = 0; i < numOfType; i++) {
        this.#myAgentRefs.push(AgentFactory.createAgent(config.type, config))
      }
    })

    while (this.#myAgentRefs.length < this.#myAgentsInfo.size) {
      this.#myAgentRefs.push(AgentFactory.createDefaultAgent())
    }
  }

  // Executes a command on a foreign peer
  remoteCall(host, cmd, data, callback) {
    let tag = host + '.' + this.#remoteIdx++ //Make unique message ID
    this.#logger.debug('Remote calling:', cmd, 'to:', host)
    this.#remoteCBs[tag] = callback //Remember what to call locally on completion
    this.#worldDBManager.insertAction(cmd, tag, host, data)
  }

  // Gets the agents from the MyCHIPs DB
  eatAgents(agents, all) {
    if (!this.#worldDBManager.isDBClientConnected()) return //Document db connected/ready

    this.#logger.trace('Loaded agents:', agents.length)
    let haveEm = [] //Keep track of which ones we've processed

    agents.forEach((agent) => {
      //For each agent in sql
      this.#allAgents.set(agent.id, agent) //Keep local copy
      if (agent.user_ent) {    //If this is one we host
        // Add to local copy of local agents info
        this.#myAgentsInfo.set(agent.id, agent)

        // Add to world db
        haveEm.push(agent.peer_cid)
        agent.random = Math.random()
        this.#worldDBManager.updateOneAgent(agent)
      }
    })

    // If loading all users (initial load)
    if (all) {
      // Delete extras in the World DB
      this.#worldDBManager.deleteManyAgents(haveEm)
      // Load up the Params and build Agent objects
      this.loadParamsConfig()
    }

    if (!this.#currAgent && this.#allAgents.size > 0) this.#currAgent = 0 //Initialize loop to traverse agents
  }
  ...
  notifyOfAgentsChange(msg) {
    this.#myChipsDBManager.queryLatestUsers(msg.time, this.eatAgents, false)
  }
  ...
  checkPeer(pData, cb) {
    ...
    let host,
      port,
      aData = this.#allAgents.get(pData.peer_cid)
    if (pData.peer_sock) {
    ...
    if (aData) cb(aData, true)
      ...
      this.#myChipsDBManager.queryPeers(pData, port, (newGuy) => {
        this.#logger.debug(
          '  Inserting partner locally:',
          newGuy.std_name,
          newGuy.peer_sock
        )
        this.#allAgents.set(newGuy.id, newGuy)
        cb(newGuy)
      })
  }
  ...
  process(run) {
    ...
    let aData = this.#allAgents.get("p100" + this.#currAgent), //Point to agent's data
      actSpace = Math.random(), //Randomize what action we will take
      startAgent = this.#currAgent
    while (!aData.user_ent) {
      if (++this.#currAgent >= this.#allAgents.size) this.#currAgent = 0
      this.#logger.trace(
      ...
      aData = this.#allAgents.get(this.#currAgent)
    }
    this.#logger.verbose(
      'Handler',
      run,
    ...
    if (++this.#currAgent >= this.#allAgents.size) this.#currAgent = 0 //Will go to next agent next time
