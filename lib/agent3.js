...
module.exports = class AgentCluster {
  ...
  configureDatabases(sqlConfig, mongoConfig, argv) {
    ...
    const sqlManager = new SQLManager(sqlConfig, this.log)
  }
  ...
  run() {
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {
      ...
      if (channel == 'parm_agent') {
        ...
        if (msg.target in this.#params && msg.value)
          this.#params[msg.target] = msg.value
    ...
    let url = `mongodb://${mongoConfig.host}:${mongoConfig.port}/?replicaSet=rs0`
    ...
    this.docClient.connect((err, client) => {
      ...
      this.docDB = client.db(mongoConfig.database)
