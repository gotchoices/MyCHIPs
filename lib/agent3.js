...
module.exports = class AgentCluster {
    ...
    run() {
        ...
        this.myChipsDBManager.createConnection(this.params, this.notifyOfAgentsChange, this.notifyOfParamsChange, this.notifyOfTallyChange);
    ...
    checkPeer(pData, cb) {
        ...
        let host, port, aData = this.agents.find((el) => el.peer_cid == pData.peer_cid);
        if (pData.peer_sock) {
            ;
    ...
    process(run) {
        ...
        while (!aData.user_ent) {
            if (++this.currAgent >= this.agents.length)
                this.currAgent = 0;
            this.logger.trace('  Skipping non-local partner:', this.currAgent, startAgent);
    ...
    tryTally(aData, pDoc) {
        ...
        this.checkPeer(pDoc, (pData, hadEm) => {
            ...
            this.remoteCall(host, 'createUser', aData, () => {
                ...
                let guid = uuidv4(), sig = 'Valid', contract = { name: 'mychips-0.99' }, tallySql = 'insert into mychips.tallies_v (tally_ent, tally_guid, partner, user_sig, contract, request) values ($1, $2, $3, $4, $5, $6);', partner = 'test';
                this.logger.debug('Tally request:', tallySql, aData.id, pData.id);
                this.myChipsDBManager.query(tallySql, [aData.id, guid, pData.id, sig, contract, 'draft'], (err, res) => {
                    if (err) {
                        this.logger.error('In query:', err.stack);
                        return;
                    }
                    this.logger.debug('  Initial tally by:', aData.std_name, ' with partner:', pData.std_name);
                    aData.stocks++;
                    //          pData.foils++
                });
            });
    ...
    payVendor(aData, vIdx, vData) {
        let guid = uuidv4(), sig = 'Valid', max = Math.max(aData.units * this.params.maxtopay, 1000), //Pay 1 CHIP or % of net worth
