...
const userFields = ['id', 'std_name', 'peer_cuid']
const parmQuery = "select parm, value from base.parm_v where module = 'model'"
const userQuery = `select id, std_name, peer_cuid, stocks, foils, part_cuids, vendors, clients,
        mychips.user_cert(id) as cert
	from mychips.users_v_tallysum where user_ent notnull and peer_agent notnull`
  ...
  process() {						//Do transactions for a single entity
    ...
    if (aData.stocks <= 0 || actSpace < this.parms.addclient) {				//I don't have any clients (or a job)
      ...
      for(cData = clients[clIdx]; clIdx < clients.length; cData = clients[clIdx++])
        if (aData.id != cData.id && !aData.part_cuids.includes(cData.peer_cuid)) break		//Don't link to myself or the same person twice
      this.log.debug("  attempt to ask:", cData.std_name, "to be my client", aData.stocks, cData.foils, clIdx)
      if (clIdx < clients.length && aData.stocks < this.parms.maxstocks && cData.foils < this.parms.maxfoils)
      ...
      for(vData = vendors[vdIdx]; vdIdx < vendors.length; vData = vendors[vdIdx++])
        if (aData.id != vData.id && !aData.part_cuids.includes(vData.peer_cuid)) break		//Don't link to myself or the same person twice
      this.log.debug("  attempt to ask:", vData.std_name, "to be my vendor", vData.stocks, aData.foils, vdIdx)
      if (vdIdx < vendors.length && vData.stocks < this.parms.maxstocks && aData.foils < this.parms.maxfoils)
...
//console.log('date:', date, 'uuid:', uuid)
  ...
  createTally(vData, cData) {					//We are not asking--just forcing (which would normally be illegal) an open tally into the DB
    let uuid = this.mkUuid(cData.peer_cuid, cData.peer_agent)
      , sig = "Valid"
      , sql = `insert into mychips.tallies (tally_type, status,
            tally_ent, tally_uuid, part_ent, part_cert, hold_sig, part_sig
          ) values 
          ('stock','open', $1, $2, $3, $4, $5, $6),
          ('foil' ,'open', $7, $8, $9,$10,$11,$12) returning tally_seq;`
      , parms = [vData.id, uuid, cData.id, cData.cert, sig, sig,
                 cData.id, uuid, vData.id, vData.cert, sig, sig]
//this.log.debug("SQL:", sql, JSON.stringify(parms))
  ...
  payVendor(aData, vData) {
    let uuid = this.mkUuid(aData.peer_cuid, aData.peer_agent)
      , sig = "Valid"
