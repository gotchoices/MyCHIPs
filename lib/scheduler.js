...
this.log.debug("Running lift check:", cont.parms.limit)
    let sql = 'select mychips.lift_next_user($1);'
    if (cont.parms.limit > 0) this.db.query(sql, [cont.parms.limit], (err, res) => {
...
this.log.debug("CHIP Value Handler:", cont)
    this.db.query(checkCHIPSql).then(res => {
      let row = res?.rows[0] ?? {}
        , samp = new Date(row?.sample)
        , sampSecs = samp.getTime() / 1000
        , stale = new Date ((sampSecs + staleSecs) * 1000)
        , today = new Date()					//;this.log.debug("  CHIPsample:", today, stale)
      if (row.sample && today < stale) return
      Fetch(chipURL).then(res => res.json()).then(res => {	//this.log.debug('chip:', res)
        let { date, base, value } = res
          , sql = 'insert into base.exchange (curr, base, rate, sample) values ($1, $2, $3, $4) on conflict do nothing;'
