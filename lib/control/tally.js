...
function invite(info, cb, resource) {		//Build a tally invitation: uses express server for included svg resource
  ...
  return Ticket(info, sql, ['tally_seq'], ['reuse'], (error, ticket) => {
    let makeContent = function(format) {
       let url = encodeURI(`https://mychips.org/invite?token=${ticket.token}&chad=${JSON.stringify(ticket.chad)}`)
          switch (format) {
            case 'json':
              return {
                invite: ticket, url,
...
//              return qr.imageSync(JSON.stringify({invite:ticket}), {type:'svg'})
            case 'link':
