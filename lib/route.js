...
//log.debug("In draft handler:", JSON.stringify(msg))
...
//log.debug("In good handler:", JSON.stringify(pmsg))
...
//log.trace("in none handler:", JSON.stringify(msg))
        ...
        delete pmsg.step; delete pmsg.reverse
//log.debug("In none handler:", JSON.stringify(pmsg))
      this.peerTransmit(pmsg, null, e=>this.peerError(msg,e))
    },
  },
...
//log.debug("In query handler:", servSock, JSON.stringify(msg))
      this.dbProcess(msg, {
        context:	['null','void','timeout','unknown'],
        query:		''
      }, result=>{
        let [ action, settings ] = result.split('|')		//Get payload from response, if any
//log.debug("Query action:", result, 'a:', action, 's:', settings)
        if (settings) try {settings = JSON.parse(settings)} catch(e) {settings = {}}
        if (action == 'affirm' || action == 'fail') {	//Return success/failure to sender without further state action (there will be no local route record to act upon or trigger from)
          let pmsg = Object.assign({}, msg, {at:msg.here, here:msg.at, action, settings})
...
// Fixme: Insert more validity checks/limits here
      let settings = (({min,max,margin,reward})=>({min,max,margin,reward})) (msg.settings || {})
log.debug("Got affirm:", JSON.stringify(msg), "sets:", settings)
      this.dbProcess(msg, {
        context: ['pend', 'timeout', 'unknown'],
        update: Object.assign({status: 'good'}, settings)
      }, null, e=>this.dbError(msg,e))
    },
    fail: function(msg) {			//The upstream partner says the route fails somehow
// Fixme: Insert validity checks/limits here
//log.debug("In fail:", JSON.stringify(msg))
      this.dbProcess(msg, {
        context: ['pend', 'timeout', 'unknown'],
        update: {status: 'none'}
      }, null, e=>this.dbError(msg,e))
