...
//X- Multiple agent instances (one on each machine) can all access same mongodb
//X- Rely on hostID for distinct server instances?
//X- Do I need an ssl gateway for other hosts to reach my VM?
//X- Find my transactions in mongo--not in postgres
...
const userQuery = `select id, std_name, ent_name, fir_name, ent_type, user_ent,
	...
	coalesce(s.partner_cdis,'{}'::text[]) as partners,
	coalesce(s.vendor_cdis,'{}'::text[]) as vendors,
	coalesce(s.client_cdis,'{}'::text[]) as clients
...
module.exports = class AgentCluster {
  constructor(sqlConfig, docConfig, argv) {
    ...
    let dbConf = Object.assign({
      log: this.log,
      listen: ['mychips_agent', 'mychips_admin', 'mychips_user'],
    }, sqlConfig)

    this.host = Os.hostname()
    this.log.info('Initializing agent model controller 2 on:', this.host)
    this.parms = {interval: argv.interval, addclient: 0.10, addvendor: 0.20}
    this.agents = []
    this.agent = null
    this.counter = 0
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {	//Initialize Database connection
      ...
      if (channel == 'mychips_agent') {
        ...
        if (msg.target == 'peers' || msg.target == 'tallies') {
          this.sqlDB.query(userQuery + " and s.mod_date >= $1", [msg.time], (e,r)=>{this.eatAgents(e,r)})
        }
