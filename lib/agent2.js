...
module.exports = class AgentCluster {
  constructor(sqlConfig, docConfig, argv) {
    ...
    this.parms = {
      ...
      addclient: 0.1, //Try to add client 10% of the time
      checksets: 0.5, //Try to adjust settings 50% of the time
      addvendor: 0.4, //Try to add vendors 15% of the time (not yet implemented)
      maxstocks: 2,
      maxfoils: 3,
      mintotpay: -10000, //Make payments if net worth greater than this
      maxtopay: 0.1, //Pay up to 10% of my net worth
      maxtarget: 100,
    }
    this.agents = []
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {
    ...
    this.sqlDB.query(parmQuery, (e, r) => {
      this.eatParms(e, r)
  ...
  process(run) {
    ...
    if (
      ...
      //
    } else if (aData.foils > 0 && aData.units > this.parms.mintotpay) {
