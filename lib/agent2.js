...
//	coalesce(s.partner_cdis,'{}'::text[]) as partner_cdis,
//	coalesce(s.vendor_cdis,'{}'::text[]) as vendor_cdis,
//	coalesce(s.client_cdis,'{}'::text[]) as client_cdis,
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {	//Initialize Database connection
      ...
      if (channel == 'parm_agent') {
        ...
        if (msg.target == 'peers' || msg.target == 'tallies') {
          this.sqlDB.query(userQuery + " and latest >= $1", [msg.time], (e,r)=>{this.eatAgents(e,r)})
        }
...
this.log.verbose("  stocks:", aData.stocks)
