...
module.exports = class AgentCluster {
  constructor(sqlConfig, docConfig, argv) {
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {	//Initialize Database connection
      var msg
      this.log.debug('Agent DB async on:', channel, payload)
...
//      this.docAcStream = this.docAc.watch([{$match: { host: null }}])
      ...
      this.docAcStream.on('change', ev=>{
        ...
        this.log.debug("Got change:", doc.action, doc.peer)
        if (doc.action == 'createUser') {
          this.checkPeer(doc.peer, pDat=>{
            this.log.debug("Peer OK:", pDat.peer_cdi)
          })
        }
      })
...
this.log.debug("  Request to add:", aData.peer_cdi, '@', pDoc.peer_sock)
    this.docAc.insertOne({action: 'createUser', host:pDoc.peer_sock.split(':')[0], from:this.host, peer:aData}, ()=>{

this.log.debug("  Request sent:", pDoc.peer_sock)
      this.checkPeer(pDoc, pData=>{
        if (pData) requestTally(aData, pData)
      })
    })
  }

// -----------------------------------------------------------------------------
  checkPeer(pDoc, cb) {			//Make sure we have a peer in our system
    let host, port
      , aData = this.agents.find(el=>(el.peer_cdi == pDoc.peer_cdi))
    if (pDoc.peer_sock) {[ host, port ] = pDoc.peer_sock.split(':')}		//If default socket, use it for host,port

this.log.debug("Checking peer:", pDoc)
    if (!aData) this.sqlDB.query(peerSql, [pDoc.ent_name, pDoc.fir_name, pDoc.ent_type, pDoc.born_date, pDoc.peer_cdi, pDoc.peer_host||host, pDoc.peer_port||port], (err,res) => {
      if (err) {this.log.error('Adding peer:', pDoc.peer_cdi, err.stack); return}
