...
const userSql = `select id, std_name, ent_name, fir_name, ent_type, user_ent,
	peer_cdi, peer_sock, stocks, foils, partners, vendors, clients,
	vendor_cdis, client_cdis, stock_seqs, foil_seqs
	from mychips.users_v_tallysum
	where not peer_ent isnull`
const peerSql = `insert into mychips.peers_v 
	(ent_name, fir_name, ent_type, born_date, peer_cdi, peer_host, peer_port) 
...
this.log.debug("Got change:", doc.action, doc.host, doc.peer.peer_cdi, doc.peer.peer_sock)
        if (doc.action == 'createUser') {		//Someone asking me to insert a peer into the DB
          this.checkPeer(doc.peer, pDat=>{
this.log.debug("Peer added/OK:", pDat.peer_cdi, "notifying:", doc.peer.host)
...
this.log.debug("  Try tally:", aData.peer_cdi, 'with', pDoc.peer_cdi)
...
this.log.debug("  Registering for callback:", pData.peer_cdi, host)
          this.register(aData.peer_cdi,host, ()=>{	//Else wait for the confirmation from the remote host
...
// -----------------------------------------------------------------------------
  checkPeer(pData, cb) {			//Make sure we have a peer in our system
    let host, port
      , aData = this.agents.find(el=>(el.peer_cdi == pData.peer_cdi))
    if (pData.peer_sock) {[ host, port ] = pData.peer_sock.split(':')}		//If default socket, use it for host,port

this.log.debug("Checking if we have peer:", pData.peer_cdi, !!aData)
    if (aData)
      cb(pData, true)
    else if (pData.host == this.host)
      this.log.error("Should have had local user:", pData.peer_cdi)

    else this.sqlDB.query(peerSql, [pData.ent_name, pData.fir_name, pData.ent_type, pData.born_date, pData.peer_cdi, pData.peer_host||host, pData.peer_port||port], (err,res) => {
      if (err) {this.log.error('Adding peer:', pData.peer_cdi, err.stack); return}
