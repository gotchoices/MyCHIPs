...
//X- callBacks is messy, make function module that sends commands to peer servers
//X-   send transaction ID to remote
//X-   confirmation comes back using same ID
//X-   store callback in array indexed by transaction ID
//- Can I begin to model distributed lift yet?
//- Do I need to listen to mychips_user_<serv_id>?
//- 
//- From version 1:
//- Make decisions like:
...
module.exports = class AgentCluster {
  constructor(sqlConfig, docConfig, argv) {
    ...
    this.sqlDB = new dbClient(dbConf, (channel, payload) => {	//Initialize Database connection
      let msg
      this.log.debug('Agent DB async on:', channel, 'payload:', payload)
...
//      this.docAcStream = this.docAc.watch([{$match: { host: null }}])
      this.docAcStream = this.docAc.watch([{$match: { 'fullDocument.host': this.host }}])	//Get async updates
...
this.log.debug("Remote call done:", doc.tag, "from:", doc.from)
          if (this.remoteCBs[doc.tag]) {		//If I can find a stored callback
            ...
            delete this.remoteCBs[doc.tag]		//And forget about it
          }
        }
...
// -----------------------------------------------------------------------------
  remoteCall(host, cmd, data, cb) {			//Execute command on foreign peer
    let tag = host + '.' + this.remoteIdx++
this.log.debug("Remote calling:", cmd, "to:", host)
    this.remoteCBs[tag] = cb				//Remember what to call on completion
...
// -----------------------------------------------------------------------------
  process(run) {						//Iterate on a single agent
    ...
    if (!aData.user_ent) {
      this.log.debug("  Skipping non-local partner:", aData.peer_cid)
      return
    }
...
this.log.debug("Tally request:", tallySql, aDat.id, pDat.id)
...
//Just because we know about the foreign peer does not guaranty his system knows about our guy
//      if (hadEm)					//If we already had the peer in our array
//        requestTally(aData, pData)			//Go ahead with the tally
//      else 
      this.remoteCall(host, 'createUser', aData, ()=>{	//Else insert him remotely too
        requestTally(aData, pData)			//And then do the tally
      })
    })
  }
...
this.log.debug("  payVendor:", aData.id, aData.foil_seqs, seq)
    this.sqlDB.query(sql, [aData.id,seq,guid,sig,units,quid,req], (e,r) => {
