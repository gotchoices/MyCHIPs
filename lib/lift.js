...
log.debug("LE pr:", this.pm.agent, 'io:', !!inLink, !!outLink, 'ol:', JSON.stringify(outLink), 'dm:', JSON.stringify(dmsg))
        const liftRec = await new Promise(
...
//      address: {key: self.agent, cuid: self.cuid},
    ...
    const updatePeer = async (address, record) => {    //ChipNet calls here for us to send update to peer
      return this.requester.request(record, async request => {
...
log.debug("Local exec:", msg)
    ...
    const makeDmsg = () => ({target, sequence, object})		//Build message for database
//    const makePmsg = (address, request) => {		//Build message for peers
    const makePmsg = () => {		//Build message for peers
//      const to = edge.out_chad
      const from = edge.inp_chad
//      if (address.key != to.agent) throw Error("Originator agent mismatch:", to.cuid, to.agent)
      return {target, object, from}
//      return {target, object, to, from, tallies:[edge.uuid], request}
    }
    const topology = {
