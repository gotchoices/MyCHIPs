...
//log.debug("LQ pair:", Buffer.from(keyPair.publicKey).toString('base64'))
    ...
    const selfAddress = {key: self.agent, identity: self.cid}
...
log.debug("LQ noise CB:", request, linkID)
        let { to, from, tally } = chadMap[linkID]
          , pmsg = {target: 'lift', action:'query', to, from, tally, request}	//;log.debug("LQ packet send::", JSON.stringify(pmsg))
...
log.debug("LI find:", find, pm.agent)
    if (find.agent == pm.agent) {		//Attempt a local lift
      ...
      try {
        ...
        const participant = await this.getParticipant(org, where, parms, version)
...
log.debug("Remote query:", from.agent, from.cid, '->', to.agent, to.cid)
