...
log.debug("LE rel:", this.pm.agent, member)
    ...
    const trxParticipantState = new ChipNetTrx.MemoryTrxParticipantState({
      address: {key: origin.agent, cuid: origin.cuid},
      types: ['P', 'R']
    })
    const asymmetric = new ChipCrypt.AsymmetricImpl()
    const keyPair = await asymmetric.generateKeyPairBin()
    ...
    const trxParticipant = new ChipNetTrx.TrxParticipant (
      ...
      asymmetric,
      cryptoHash,
      async (address, record) => {    //chipnet asking to send update to our peer
        const to = {agent: address.key, cuid: address.cuid}
        const pmsg = { target: 'lift', action: 'update', to, from: origin }
...
this.remote_update = function(msg, pm) {	//Peer sending lift transaction
}
