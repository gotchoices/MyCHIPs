...
// -----------------------------------------------------------------------------
// TODO:
//- Implement chit release callback
//- 
...
module.exports = function(pm) {
  ...
  this.discoParticipants = {};		// Cache of discovery participants indexed by "<cuid>:<agent>"
...
//log.debug("LQ intent CB:", linkIntent, queryIntents)
    ...
    //ChipNet will call here to discover possible lift pathways
    const findAddress = async (query, inLinkId) => {
...
log.debug("Local seek:", msg)
    const { target, object, sequence, aux } = msg	//Grab message components
    const { find, units } = object
    const { cuid, agent } = find			//;log.debug("LS find:", find, 'me:', pm.agent)
    const { trans, origin } = aux			//;log.debug("LS aux:", origin, 'p:', trans)
    const dmsg = {target, sequence, object}		//Make minimal return message for DB
//log.debug("LS o:", origin, 'p:', trans, 'u:', units)
    ...
    try {
      ...
      const { plans, sessionCode } = await originator.discover({address}, {'L': {units}})
...
log.debug("LS mapPlans:", JSON.stringify(mapPlans))
  ...
  this.getTrxParticipant = async function(self, getDmsg, sessionCode) {   //Create participant instance for lift transaction
    const participantKey = `${self.cuid}:${self.agent}`		//;log.debug("Lgp pk:", participantKey)
    ...
    const nonceToLinkMap = await participantState.getNonceToLinkMap(sessionCode)
    ...
    const trxResources = {
      promise: async (member, inLinks, outLinks, record) => {     //A routine to commit local chits to the lift
        ...
        let sub, context, status, promise, insert = false;
        if (inLink && outLink) {   //intermediary
          promise = {it: inLink.linkId, ot: outLink.linkId, units}
          context = ['null']
          status = 'part'
          insert = true
        } else if (inLink) {    //terminus
          sub = {agent: inLink.link?.target?.key, cuid: inLink.link?.target?.cuid}
          promise = {it: inLink.linkId, sub, units}
          context = ['null']
          status = 'part'
          insert = true
        } else if (outLink) {   //originator
          sub = {agent: outLink.link?.source?.key, cuid: outLink.link?.source?.cuid}
          promise = {sub, ot: outLink.linkId, units}
          context = ['seek.exec']
          status = 'exec'
        }
...
log.debug("LE pr:", this.pm.agent, 'io:', !!inLink, !!outLink, 'ol:', JSON.stringify(outLink), 'dm:', JSON.stringify(dmsg))
      ...
      release: async (isSuccess, member, inLink, outLink, record) => {    //Routine to rollback/void chits
//FIXME: Call database to release chits
log.debug("LE rel:", this.pm.agent, member)
    ...
    const updatePeer = async (address, record) => {    //ChipNet calls here for us to send update to peer
      ...
      return this.requester.request(record, message => {
        const pmsg = { target: 'lift', action: 'update', to, from: self, object, request: message }
...
log.debug("Local exec:", msg)
    ...
    const trxRecord = {
      ...
      sessionCode: plan.sessionCode, payload: {lift: object}, topology, promises: [], commits: []
    }
    const trxParticipant = await this.getTrxParticipant(origin, () => dmsg, plan.sessionCode);
log.debug("LE up:")
...
log.debug("Remote update:", from.agent, from.cuid, '->', to.agent, to.cuid, msg)
    ...
    const responder = new ChipNet.Responder(
      async (body) => {		//Call here to build response to the ChipNet query
        const participant = await this.getTrxParticipant(to, () => dmsg, body.sessionCode)
