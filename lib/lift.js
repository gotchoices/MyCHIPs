...
log.debug("LE rel:", this.pm.agent, member)
        ...
        const recipe = {context: ['exec', 'part'], update: {status}}
...
log.debug("LE result:", result)
          ...
          if (newState !== status) {
            log.error(`Failed to transition to status: ${status}`, this.pm.agent)
//            throw Error()   TODO: What best to do if db update failed?
          }
        }
      }
    }
