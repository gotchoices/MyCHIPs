...
module.exports = function(pm) {
  ...
  this.getParticipant = async function(to) {
    const sql = `select inp_cid,inp,ath,out_chad,top_chad,min,max,top_uuid,uuids
                 from mychips.tallies_v_paths where inp_cid = $1 and inp_agent = $2 and foro`
    const parms = [to.cid, to.agent]			;log.debug("LQ sql:", sql, 'parms:', parms)
    const queryRes = await pm.db.query(sql, parms)	//Ask the DB for possible paths
//log.debug("LQ res:", queryRes)
    const iAmARef = false
    const version = 1
    const otherMembers = undefined
    const chadMap = {}			//Remember to/from addresses for each path
...
//log.debug("LQ pair:", Buffer.from(keyPair.publicKey).toString('base64'))
    ...
    const selfAddress = {key: to.agent, identity: to.cid}
log.debug("LQ self:", JSON.stringify(selfAddress))
    ...
    const participantOptions = new ChipNet.UniParticipantOptions(
      (request, linkID) => {			//Call back here to transmit query to the peer
log.debug("LQ noise CB:", request, linkID)
        let { to, from, tally } = chadMap[linkID]
          , pmsg = {target, action:'query', to, from, tally, request}	//;log.debug("LQ packet send::", JSON.stringify(pmsg))
...
log.debug("Local init:", JSON.stringify(msg))
    let { target, object, sequence, init, orig } = msg	//Grab message components
      , { org, auth, find, ref } = init

log.debug("LI find:", find, pm.agent)
    if (find.agent == pm.agent) {		//Attempt a local lift
      try {
        let lift_uuid = mkUuid(org?.cid, org?.agent)	//Permanent uuid
      ...
      try {
        let queryRes = await pm.db.query(sql, parms)	//Ask the DB for possible paths
...
log.debug("LI noise CB:", request, linkID)
            let { to, from, tally } = chadMap[linkID]
              , pmsg = {target, action:'query', to, from, tally, request}	//;log.debug("LI packet send::", JSON.stringify(pmsg))
...
log.debug("LI intent CB:", linkIntent, queryIntents)
        ...
        const address = {key: agent, identity: cid}
        const originatorState = await ChipNet.MemoryUniOriginatorState.build(
          originatorOptions, peerNodeLinks, asymmetricVault, cryptoHash, {address},
...
log.debug("L resp process:", this.pm.agent, JSON.stringify(body))
        const participant = await this.getParticipant(to)
        return participant.query(body, tally)
      })
...
//    this.requester.response(request)
  }
