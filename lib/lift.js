...
//log.debug("LI res:", queryRes)
        ...
        const peerNodeLinks = (queryRes?.rows ?? []).map(row => {
          let max = row.max
          let min = row.min
          chadMap[row.top_uuid] = {to: row.out_chad, from: row.top_chad}
          return {
            id: row.top_uuid,
            intents: [
...
log.debug("LI noise CB:", request, linkID)
            let { to, from } = chadMap[linkID]
              , pmsg = {target, action:'resp', to, from, request}	//;log.debug("LI packet send::", JSON.stringify(pmsg))
            
            return this.requester.request(pmsg)		//Return promise that remembers request context

//            pm.peerTransmit(pmsg, ()=>{
//log.debug("LI packet sent::", pmsg)
//            }, e=>pm.peerError(pmsg,e))
...
log.debug("Remote void:", JSON.stringify(msg, null))
  ...
  this.remote_resp = function(msg, pm) {	//Peer responding to discovery query
    const { to, from, request } = msg
//    const { messageId, body } = request
//    const response = {messageId, body}
    this.requester.response(request)
    
log.debug("Remote resp:", this.pm.agent, JSON.stringify(msg))
//    pm.dbProcess(msg, {
//      context: ['seek', 'seek.call','call','call.call', 'pend', 'pend.check'],
//      update: {request: 'void'}
//    }, null, e=>pm.dbError(msg,e))
