...
//log.debug("L req pmsg sent:", this.pm.agent, '->', pmsg.to.agent)
  ...
  this.local_init = function(msg, pm) {			//New lift has been drafted locally
...
//log.debug("LI PV:", pub, auth.sign, 'c:', core)
    crypto.verify(pub, core, auth.sign, isValid => {
      let recipe = {context: ['draft.init']}		//;log.debug("LI isValid:", isValid)
      if (isValid) {					//;log.debug("LI AV:", pm.publicKey, 'o:', object)
        noiseCrypto.sign(pm.privateKey, object, agent => {	//log.debug("LI ASs:", agent)
