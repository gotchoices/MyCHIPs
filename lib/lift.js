...
log.debug("LE rel:", this.pm.agent, member)
    ...
    const trxParticipant = new ChipNetTrx.TrxParticipant (
      ...
      async (address, record) => {    //updatePeer: ChipNet asking to send update to peer
        const to = {agent: address.key, cuid: address.cuid}
        const tally = undefined // TODO: need a way to determine target tally
        const pmsg = { target: 'lift', action: 'update', to, from: origin, tally, request: record }
...
log.debug("Local exec:", msg)
    ...
    const trxParticipant = await this.getTrxParticipant(origin);
log.debug("LE up:")
...
//log.debug("lift remote resp:", this.pm.agent, msg.from, '->', msg.to)
  ...
  this.remote_trx_update = async function(msg, pm) {	//Peer sending lift transaction
    const {to, from, target, tally} = msg
log.debug("Remote update:", from.agent, from.cuid, '->', to.agent, to.cuid)
...
//log.debug("L trx resp process:", this.pm.agent, JSON.stringify(body))
        const participant = await this.getTrxParticipant(to)
