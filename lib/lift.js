...
//log.debug("Local local:", JSON.stringify(msg))
      let o = msg.object
        , pmsg = Object.assign({}, msg, {action: 'terminus', at: o.socket, fat: msg.at})	//At certified lift origin socket
log.debug("Local lift local dest:", JSON.stringify(pmsg))
      this.peerTransmit(pmsg, ()=>{		//Action: 
        this.dbProcess(msg, {
          context:	['local'],
          update:	{status: 'pend'}
...
log.debug("Remote terminus:", JSON.stringify(msg))
      this.dbProcess(msg, {
        context: ['pend', 'seek', 'timeout'],
        update: {status: 'good', signature: 'Magic signature'}
      }, result => {
        let pl; try {pl = JSON.parse(result)} catch(e) {pl = {}}
        let o = msg.object
          , pmsg = Object.assign({}, msg, {action: 'commit', at: pl.return, fat:msg.at, from: pl.from, user: pl.user, signature: pl.signature})
