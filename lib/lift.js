...
module.exports = function(pm) {
  ...
  this.getParticipant = async function(self, where, parms) {
    ...
    const getPeerState = async () => {
      const sql = `select inp_cuid,inp,ath,out_chad,top_chad,min,max,top_uuid,uuids from mychips.tallies_v_paths where ${where}`
...
log.debug("LI plans:", plans)
        if (plans.length) {
          ...
          // Discovery failed, no plans found
        }
