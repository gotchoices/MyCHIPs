...
var log = Log('tally')
var subCrypto = new SubCrypto(log)
...
module.exports = function(pm) {
  ...
  this.signCheck = function(tally, passCB, failCB) {
    ...
    if (fSign && !this.test) {				//log.debug('check fSign:', fKey)
      pChain = pChain.then(() => {
        return subCrypto.verify(fKey, message, fSign).then(isValid => {
          if (!isValid) {
    ...
    if (sSign && !this.test) {				//log.debug('check sSign:', sKey)
      pChain = pChain.then(() => {
        return subCrypto.verify(sKey, message, sSign).then(isValid => {
          if (!isValid) {
...
log.debug('Local offer:', msg.from?.cuid, '->', msg.to?.cuid, msg.to?.agent, msg.action)
...
log.debug('Local draft:', msg.from.cuid, '->', msg.to.cuid, msg.to.agent, msg.action)
    let pmsg = Object.assign({}, msg)
      , dmsg = {action:msg.action, target:msg.target, to: msg.from, object: {uuid:msg.object.uuid}}
...
log.debug('Local void:', msg.from.cuid, '->', msg.to.cuid, msg.to.agent, msg.action)
    let pmsg = Object.assign({}, msg)
      , dmsg = {action:msg.action, target:msg.target, to: msg.from, object: {uuid:msg.object.uuid}}
...
log.debug('Local open:', msg.from.cuid, '->', msg.to.cuid, msg.to.agent, msg.action)
    let pmsg = Object.assign({}, msg)
      , dmsg = {action:msg.action, target:msg.target, to: msg.from, object: {uuid:msg.object.uuid}}
...
log.debug('Remote offer:', msg.from.cuid, msg.from.agent, '->', msg.to.cuid, msg.action)
    this.signCheck(msg.object, () => {		//Are any given signatures valid?
      pm.dbProcess(msg, {
...
log.debug('Remote draft:', msg.from.cuid, msg.from.agent, '->', msg.to.cuid, msg.action)
...
log.debug('Remote void:', msg.from.cuid, msg.from.agent, '->', msg.to.cuid, msg.action)
    let dmsg = {action:msg.action, target:msg.target, to: msg.to, object: {uuid: msg.object.uuid}}
    pm.dbProcess(dmsg, {
      context: ['H.offer'],
      update: {status: 'void'}
...
log.debug('Remote open:', msg.from.cuid, msg.from.agent, '->', msg.to.cuid, msg.action)
    this.signCheck(msg.object, () => {		//Are any given signatures valid?
      pm.dbProcess(msg, {
        context: ['H.offer'],
