...
//  userAccept:		status=draft,	request=open,	both signatures
...
log.debug('Local ticket:', msg)
      let {chip, token, cert} = msg
        , {cid, host, port, agent} = chip
        , at = [host,port].join(':')
        , ticket = {token, cert, key: agent}
        , pmsg = this.screen(msg, {at, ticket})
log.debug('local ticket:', pmsg)
