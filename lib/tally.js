...
// -----------------------------------------------------------------------------
//- TODO:
//- Modify signCheck to have a validCB and invalidCB
//- argument tells which signature failed
//- 
...
log.debug('Remote offer:', msg.from.cid, msg.from.agent, '->', msg.to.cid, msg.action)
      signCheck(msg.object, valid => {		//Are any given signatures valid?
        if (valid) {
          ...
          this.dbProcess(dmsg, {		//User will at least get notified
            context: ['null','void','H.offer','P.offer'],
            update: {},
            notify: true
          }, null, e=>this.dbError(dmsg,e))
...
//log.debug('  ro:', JSON.stringify(cmsg))
            if (tally && hash)
              this.peerTransmit(cmsg, null, e=>this.peerError(pmsg,e))	//start chain consensus
