...
module.exports = class PeerCont {
  constructor(servConfig, dbConfig) {
    let { host, port, key } = servConfig
      ...
      , log = this.log = Log('peer' + (agentPub ? '_' + agentPub.slice(0,4) : ''))
      , dbConf = {
          log,
    ...
    this.peerNoise = new PeerNoise({host, port, key, log, queryCB:this.query}, (serv, msg) => {
...
this.log.debug("Peer query:", req, data)
  }
