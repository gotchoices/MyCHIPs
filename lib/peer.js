...
module.exports = class PeerCont {
  constructor(servConfig, dbConfig) {
    let { host, port, key } = servConfig
      , agentPub = key ? key.publicKey : null
      , log = this.log = servConfig.log || Log('peer' + (agentPub ? '_' + agentPub.slice(-4) : ''))
      , dbConf = {
          log,
          listen: ['parm_peers', 'mychips_peer' + (agentPub ? '_' + agentPub : '')],
        }	
    ...
    this.log.info('Initializing peer server at:', host, port, 'Agent:', key)

    this.peerNoise = new PeerNoise({host, port, key, log, queryCB:(r,d,c)=>this.query(r,d,c)}, (serv, msg) => {
...
//if (Math.random() < 0.25) {failureCB(); return}		//Purposely fail for testing retries
    this.peerNoise.send(msg.at, msg, successCB, failureCB)
  }
...
this.log.debug("Noise peer query:", req, data)
    if (req == 'ticket') {
      let { token, cert } = data
        , sql = Format('select mychips.token_valid(%L, %L);', token, cert)
      this.db.query(sql, (e, r)=>{
        if (e) {this.log.error('Error in token query:', e.message); return}
this.log.debug('Query res:', r)
//        r.rows.forEach(row => {
//          let { parm, value } = row
//          this.parms[parm] = value
//        })
