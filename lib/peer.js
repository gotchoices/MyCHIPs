...
module.exports = class PeerCont {
  constructor(port, hostID, config) {
    this.port = port
    this.hostID = hostID
    this.log = require('./logger')('peer' + (hostID ? '-'+hostID : ''))
...
//this.log.trace("DB status:", this.db.client.activeQuery)
  ...
  dbProcess(msg, dbLogic, successCB, failureCB) {		//Call the database with state traffic
    ...
    this.db.query(`select mychips.${msg.target}_process($1,$2);`, [msg, JSON.stringify(dbLogic)], (err, res) => {
      if (err) {
  ...
  userNotify(msg) {						//Notify our user of updates
    this.log.info("Notify user of:", msg)
  }
