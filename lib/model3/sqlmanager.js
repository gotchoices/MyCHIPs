...
class SQLManager {
    ...
    addConnectionRequest(requestingAccountID, requestingAccountCertificate, targetAccountCertificate) {
        ...
        this.query('insert into mychips.tallies_v (tally_ent, tally_uuid, contract, request, hold_cert, part_cert, hold_sig, tally_type) values ($1, $2, $3, $4, $5, $6, $7, $8);', [
            ...
            if (err) {
                ...
                console.log('Error while making tally between', requestingAccountCertificate.chad.cid, 'and', targetAccountCertificate.chad.cid);
                console.log(err);
                return;
            }
            console.log('Tally offered between', requestingAccountCertificate.chad.cid, 'and', targetAccountCertificate.chad.cid, ':', uuid);
    ...
    updateConnectionRequest(entity, sequence, accepted) {
        if (accepted) {
            let target = 1;
            this.query("update mychips.tallies_v set request = 'open', hold_sig = $3, target = $4 where tally_ent = $1 and tally_seq = $2 returning state;", [entity, sequence, 'Accepted', target], (err, res) => {
                if (err) {
                ...
                this.logger.verbose('Tally accepted:', row.tally_ent, row.tally_seq, row.state);
            });
        }
        else {
    ...
    addPayment(spenderId, receiverId, chipsToSpend, sequence) {
        ...
        this.query('insert into mychips.chits_v (chit_ent,chit_seq,chit_uuid,signature,units,quidpro,request) values ($1,$2,$3,$4,$5,$6,$7)', [spenderId, sequence, (0, uuid_1.v4)(), 'Signed', chipsToSpend, quid, 'good'], (e, r) => {
            if (e) {
