...
class FindNewSpendingTarget {
    ...
    run() {
        if (this.account.numSpendingTargets <= 0 || // If the account has no stocks   or
            ...
            this.worldDBManager.findPeerAndUpdate(this.account.peer_cuid, this.account.spendingTargetCids, (newPeer) => {
                this.logger.debug(this.account.peer_cuid, '  attempting new spending source with', newPeer.peer_cuid);
