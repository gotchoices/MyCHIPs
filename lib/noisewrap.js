...
module.exports = class NoiseWrap {
  constructor(config) {
    ...
    this.log = config.log
    this.raw = config.raw		//Disable encryption for testing/simulations
...
this.log.debug("Key check:", state.rs, key)
  ...
  decrypt(split, decryptBuf, messageBuf) {
    if (!this.raw) try {
      ...
      throw e  // Rethrow to allow proper handling up the chain
    } else {
  ...
  edToX(edKeys) {
    let privateKey
      , publicKey
    try {			//this.log.debug("Conv:", JSON.stringify(edKeys))
      ...
      this.log.error('Convert EdToX: ' + err.message)
      throw err
    }
  }
}
