...
module.exports = class PeerCont {
  constructor(servConfig, dbConfig) {
    let { host, port, keys } = servConfig
      , agentPub = keys ? keys.publicKey : null
      , log = this.log = servConfig.log || Log('peer' + (agentPub ? '_' + agentPub.slice(-4) : ''))
      , dbConf = {
          log,
          listen: ['parm_peers', 'mychips_agent' + (agentPub ? '_' + agentPub : '')],
        }	
    ...
    this.peerNoise = new PeerNoise({host, port, keys, log, queryCB:(r,d,c)=>this.query(r,d,c)}, (serv, msg) => {
...
//this.log.trace("Database handler in peer2peer.js", msg, JSON.stringify(dbLogic))
  ...
  peerTransmit(msg, successCB, failureCB) {			//Transmit state traffic to the partner
    this.log.trace("In function peerTransmit in peer2peer.js", msg)
