  ...
  constructor(servConfig, dbConfig) {
    ...
    this.peerNoise = new PeerNoise({
      ...
      if (msg.target in this.targets) {			//If we have a handler for the specified target (tally, chit, etc.)
        ...
        if (typeof target[method] === 'function') try {
          target[method](msg, this)		//Call state handler
        } catch (err) {
...
this.log.debug("X:", Object.keys(target), method, typeof target[method])
        if (typeof target[method] === 'function') try {
          target[method](msg, this)				//Call state handler
        } catch (err) {
          this.log.error("Processing local packet:", err.message)
        }
      }
    ...
    this.closeDB=false
    this.targets = {}
    this.targets.tally = new Tally(this.test)
    this.targets.chit = new Chit(this.test)
    this.targets.route = new Route()
    this.targets.lift = new Lift()
