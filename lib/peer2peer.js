...
//- 
const Os = require('os')
const NoiseCrypto = require('./noisecrypto')
...
module.exports = class PeerCont {
  constructor(servConfig, dbConfig) {
    ...
    this.noiseCrypto = new NoiseCrypto(log)
    this.agent = agent
    this.keys = keys				;log.debug('keys:', keys)
    this.npKeys = this.noiseCrypto.convertEdToX(keys)
...
//this.log.debug('Query res:', r, row)
      ...
      let edKeys = {publicKey: Buffer.from(data, 'base64url')}
        , { publicKey } = this.noiseCrypto.convertEdToX(edKeys)
