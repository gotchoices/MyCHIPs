...
module.exports = class LiftScheduler {
  constructor(config) {
    this.log = require('./logger')('lifts')
    this.log.info('Initializing lift scheduling controller')
    let dbConf = {
      logger: this.log,
      listen: ['mychips_lifts'],
    }
    this.parms = {interval: 10000, limit:1, sort1: 'length', sort2: 'min'}
    if (config) Object.keys(this.parms).forEach(key=>{if (key in config) this.parms[key] = config[key]})
