...
module.exports = function(pm) {
  ...
  this.signCheck = function(info, passCB, failCB) {
    ...
    const doVerify = (pubKey, msg, signature) => {	//Validate or pass
      ...
      return subCrypto.verify(pubKey, msg, signature).then(isValid => {
        if (!isValid) {
