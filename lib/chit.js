...
//- 
// A chit describes an individual amount of CHIPs owed from one person to 
// another and recorded on an associated tally.  A tally would typically contain 
...
//    delete pmsg['entity']; delete pmsg['peer']
  ...
  peerDecline: function(msg) {		//The partner will not make payment
    this.dbProcess(msg, {context: ['userInvoice'], update: {signature: 'declined'}}, ()=>{
      this.userNotify(msg)
    }, ()=>{log.trace('Failure in Chit state: peerDecline')})
  },
  peerTimeout: function(msg) {		//Invoice has been out too long
    this.dbProcess(msg, {context: ['userInvoice'], upsert: {request: 'userRequest'}}, ()=>{
      this.userNotify(msg)
    }, ()=>{log.trace('Failure in Chit state: peerTimeout')})
  },
  peerValid: function(msg) {		//The partner agrees to the outstanding invoice
    this.dbProcess(msg, {context: ['null','userInvoice'], upsert: ''}, ()=>{
      this.userNotify(msg)
    }, ()=>{log.trace('Failure in Chit state: peerValid')})
  },
  peerInvoice: function(msg) {		//The partner has sent us a request for payment
    this.dbProcess(msg, {context: ['null', 'userVoid'], upsert: ''}, ()=>{
      this.userNotify(msg)
    }, ()=>{log.trace('Failure in Chit state: peerInvoice')})
  },
  ...
  userDraft: function(msg) {		//The user is sending money voluntarily
    var pmsg = Object.assign({}, msg, {action: 'peerDeposit', user: msg.peer})
    delete pmsg['entity']; delete pmsg['peer']
    this.peerTransmit(pmsg, ()=>{
