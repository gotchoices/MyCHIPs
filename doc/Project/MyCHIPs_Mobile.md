...
- Phase 1: Provider Connection / Authentication
  ...
  - Generate and store local connection key
  - Perform key exchange/authentication with provider/server
  ...
  - Demonstrate that backend packets can be received and interpreted
  - User can select preferred language
  - Language display data derived from backend (in user's language of choice)
- Phase 2: Tally Initiation
  - App generates user signing key if not already present
  - Tally Initiation button sends request packet to backend
  - (Backend will generate a tally request QR for you)
...
- Phase 3: Tally Negotiation
  ...
  - Take user to tally review screen
...
- Phase 5: Settings/Configuration
  ...
  - Local connection key and signing key stored encrypted, validated with
    - Passphrase, or
    - Phone supported biometrics
